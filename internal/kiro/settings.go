...
const (
	// Autonomous roles (polecat, witness, refinery, deacon) need mail in SessionStart
...
// EnsureSettings ensures .kiro/settings.json exists in the given directory.
func EnsureSettings(workDir string, roleType RoleType) error {
	return EnsureSettingsAt(workDir, roleType, ".kiro", "settings.json")
}
...
func EnsureSettingsAt(workDir string, roleType RoleType, settingsDir, settingsFile string) error {
	kiroDir := filepath.Join(workDir, settingsDir)
	settingsPath := filepath.Join(kiroDir, settingsFile)
